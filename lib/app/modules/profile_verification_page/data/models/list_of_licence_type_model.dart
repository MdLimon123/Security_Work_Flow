// /// success : true
// /// message : "data fatched!"
// /// licence_types : [{"id":1,"title":"Security Agent License"},{"id":2,"title":"Crowd Controller License"},{"id":3,"title":"Investigator License"},{"id":4,"title":"Security Officer License"},{"id":5,"title":"Bodyguard License"}]
library;

// class ListOfLicenceTypeModel {
//   ListOfLicenceTypeModel({
//     bool? success,
//     String? message,
//     List<LicenceTypes>? licenceTypes,
//   }) {
//     _success = success;
//     _message = message;
//     _licenceTypes = licenceTypes;
//   }

//   ListOfLicenceTypeModel.fromJson(dynamic json) {
//     _success = json['success'];
//     _message = json['message'];
//     if (json['licence_types'] != null) {
//       _licenceTypes = [];
//       json['licence_types'].forEach((v) {
//         _licenceTypes?.add(LicenceTypes.fromJson(v));
//       });
//     }
//   }
//   bool? _success;
//   String? _message;
//   List<LicenceTypes>? _licenceTypes;
//   ListOfLicenceTypeModel copyWith({
//     bool? success,
//     String? message,
//     List<LicenceTypes>? licenceTypes,
//   }) => ListOfLicenceTypeModel(
//     success: success ?? _success,
//     message: message ?? _message,
//     licenceTypes: licenceTypes ?? _licenceTypes,
//   );
//   bool? get success => _success;
//   String? get message => _message;
//   List<LicenceTypes>? get licenceTypes => _licenceTypes;

//   Map<String, dynamic> toJson() {
//     final map = <String, dynamic>{};
//     map['success'] = _success;
//     map['message'] = _message;
//     if (_licenceTypes != null) {
//       map['licence_types'] = _licenceTypes?.map((v) => v.toJson()).toList();
//     }
//     return map;
//   }
// }

// /// id : 1
// /// title : "Security Agent License"

// class LicenceTypes {
//   LicenceTypes({num? id, String? title}) {
//     _id = id;
//     _title = title;
//   }

//   LicenceTypes.fromJson(dynamic json) {
//     _id = json['id'];
//     _title = json['title'];
//   }
//   num? _id;
//   String? _title;
//   LicenceTypes copyWith({num? id, String? title}) =>
//       LicenceTypes(id: id ?? _id, title: title ?? _title);
//   num? get id => _id;
//   String? get title => _title;

//   Map<String, dynamic> toJson() {
//     final map = <String, dynamic>{};
//     map['id'] = _id;
//     map['title'] = _title;
//     return map;
//   }
// }

class ListOfLicenceTypeModel {
  bool? success;
  String? message;
  List<LicenceTypes>? licenceTypes;

  ListOfLicenceTypeModel({this.success, this.message, this.licenceTypes});

  ListOfLicenceTypeModel.fromJson(Map<String, dynamic> json) {
    success = json['success'];
    message = json['message'];

    if (json['licence_types'] != null) {
      licenceTypes = (json['licence_types'] as List)
          .map((e) => LicenceTypes.fromJson(e as Map<String, dynamic>))
          .toList();
    } else {
      licenceTypes = [];
    }
  }

  Map<String, dynamic> toJson() {
    return {
      'success': success,
      'message': message,
      'licence_types': licenceTypes?.map((e) => e.toJson()).toList(),
    };
  }
}

class LicenceTypes {
  int? id;
  String? title;
  String? stateOrTerritory;

  LicenceTypes({this.id, this.title, this.stateOrTerritory});

  LicenceTypes.fromJson(Map<String, dynamic> json) {
    id = json['id'];
    title = json['title'];

    stateOrTerritory = json['state_or_territory'];
  }

  Map<String, dynamic> toJson() {
    return {'id': id, 'title': title, 'state_or_territory': stateOrTerritory};
  }
}
